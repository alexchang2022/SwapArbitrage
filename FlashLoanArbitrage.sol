...
contract FlashLoanArbitrage is Test {
  ...
  constructor(address _uniSwapRouter, address _sushiSwapRouter) payable {

  	// create uniswap Router 
	uniSwapRouter = IUniswapV2Router02(_uniSwapRouter);
	factory = IUniswapV2Router02(uniSwapRouter).factory();
...
function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
  ...
  //uint amountRequired = UniswapV2Library.getAmountsIn(factory, amountTokenBorrowed, rpath)[0]; 
  uint amountRequired = uniSwapRouter.getAmountsIn(amountTokenBorrowed, rpath)[0]; 
  ...
  uint amtPreCal = sushiSwapRouter.getAmountsOut(amountTokenBorrowed, path)[2];

  uint amountReceived = sushiSwapRouter.swapExactTokensForTokens( amountTokenBorrowed, amountRequired, path, address(this), deadline)[2]; 
